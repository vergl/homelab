resource "proxmox_vm_qemu" "proxmox_vm_docker" {
  count       = 1
  vmid        = "701"
  name        = "docker"
  desc        = "Node for docker containers"
  target_node = var.pm_node_name
  clone       = var.template_vm_name
  os_type     = "cloud-init"
  agent       = 1
  memory      = var.docker_vm_memory
  cores       = var.docker_vm_cores
  boot        = "order=virtio0;ide2;net0;ide0"
  onboot      = true
  disk {
    size     = var.docker_vm_disk_size
    type     = var.docker_vm_disk_type
    storage  = var.docker_vm_storage
    iothread = 1
  }

  ipconfig0 = "ip=${var.docker_vm_ip}/${var.network_range},gw=${var.network_gateway}"
  ciuser    = "stanislavgiley"
  sshkeys   = <<EOF
  ssh-rsa  stanislavgiley@MacBook-Pro-Stanislav-2.local 
  EOF

  # We have to manually reboot our VMs to make them have IP address we set in ipconfig0 (it get IP address from DHCP on first boot)
  provisioner "remote-exec" {
    inline = [
      "sudo apt update",
      "sudo DEBIAN_FRONTEND=noninteractive apt install -y ansible python3-pip",
      "sudo shutdown -r +0"
    ]
    connection {
      type     = "ssh"
      host     = self.ssh_host
      user     = var.ssh_user
      password = var.ssh_password
      port     = self.ssh_port
    }
  }

  lifecycle {
    ignore_changes = [
      ciuser,
      sshkeys,
      network
    ]
  }
}
